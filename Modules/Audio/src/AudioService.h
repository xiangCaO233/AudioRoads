...
/// 后端与图的析构顺序明确：先销毁纯值图，再释放可能调用平台 API 的后端。
/// 流运行后应先停止回调，再替换快照或销毁服务，不能依赖成员自然析构竞态。
...
public:
    ...
    /// @warning 引用不得超过服务寿命，也不能交给平台实时回调长期保存。
    [[nodiscard]] Core::RoutingGraph& routingGraph() noexcept;

    /// @brief 返回只读路由图。
    /// @note 仍是控制面借用引用，不构成可原子发布的实时执行快照。
